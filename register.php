...
if (isset($_POST["regBtn"])) {
  ...
  function checkword($password, $words = array(), $offset = 0)
  {
    foreach ($words as $w) {
      if (stripos($password, $w, $offset) !== false) {
        return true;
      }
    }
    return true;
    ...
    if (checkname($pass, explode(" ", $fname)) && checkname($pass, explode(" ", $lname)) && (stripos($pass, $uName) === false)) {
      if ($pass == $cPass) {
        if (checkword($pass, $wordsarray)) {
          ...
          if ($row = $result->fetch_array()) {
            ...
            if ($gender != "null") {
              ...
              if ($conn->query($sql) == true) {
                ...
                echo "<script>alert('Registered Successfully $to');window.location='login.php';</script>";
              } else {
                echo $conn->error;
              }
            } else {
          ...
          echo "<script>alert('Password should not contain words from dicionary');</script>";
        }
      } else {
